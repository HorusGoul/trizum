...
```bash
vp exec react-doctor rules explain react-doctor/no-array-index-as-key
```
...
```bash
vp exec react-doctor --verbose --diff
```

## Commands

```bash
vp exec react-doctor rules list                         # every rule + its effective severity
vp exec react-doctor rules list --configured            # only what your config changed
vp exec react-doctor rules list --category Performance  # filter by category
vp exec react-doctor rules explain <rule>               # why it matters + how to configure
vp exec react-doctor rules disable <rule>               # rule never runs
vp exec react-doctor rules enable <rule>                # turn back on at its recommended severity
vp exec react-doctor rules set <rule> warn              # off | warn | error
vp exec react-doctor rules category "React Native" off  # whole category
vp exec react-doctor rules ignore-tag design            # skip a rule family (design, test-noise, ...)
vp exec react-doctor rules unignore-tag design
```
