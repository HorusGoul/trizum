...
export function DestinationPartyCard({
  ...
  return (
    <button
      type="button"
      className={cn(
        "w-full rounded-xl border p-4 text-left shadow-xs transition-all duration-200 ease-in-out",
        "border-accent-200/80 bg-gradient-to-br from-white via-white to-accent-50/80 active:scale-[0.99] hover:border-accent-300/90 hover:shadow-md focus-visible:border-accent-500 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-accent-500/20 dark:border-accent-800 dark:from-accent-950 dark:via-accent-950 dark:to-accent-900/70 dark:shadow-none dark:hover:border-accent-700 dark:focus-visible:border-accent-500",
      )}
      onClick={onPress}
    >
      <div className={cn("flex gap-4", hasSupportingCopy ? "items-start" : "items-center")}>
        <PartySymbolBadge party={option.entry.party} className="mt-1 h-12 w-12 text-xl shadow-xs" />
