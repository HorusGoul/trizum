#!/bin/bash

set -euo pipefail

export SENTRY_ORG="horusdev"
export SENTRY_PROJECT="trizum-pwa"

# This script is the package build entry point; callers should use `vp run build`.
vp build .

sentry-cli sourcemaps inject ./dist/client

if [ -z "${SENTRY_AUTH_TOKEN:-}" ]; then
  echo "SENTRY_AUTH_TOKEN is not set, skipping sourcemaps upload"
  exit 0
fi

# Get full version
PWA_VERSION=$(node -p "require('./package.json').version")
LAST_COMMIT="$(git rev-parse --short HEAD)"
FULL_VERSION="${PWA_VERSION}-${LAST_COMMIT}"

echo "Uploading sourcemaps to Sentry for release $FULL_VERSION. Org: $SENTRY_ORG, Project: $SENTRY_PROJECT"

sentry-cli sourcemaps upload \
  --release "$FULL_VERSION" \
  --org "$SENTRY_ORG" \
  --project "$SENTRY_PROJECT" \
  ./dist/client
