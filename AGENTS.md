...
This project is using Vite+, a unified toolchain built on top of Vite, Rolldown, Vitest, tsdown, Oxlint, Oxfmt, and Vite Task. Vite+ wraps runtime management, package management, and frontend tooling in a single global CLI called `vp`. Vite+ is distinct from Vite, and direct app commands use `vp dev` and `vp build`; in this repo, build workflows should call package scripts with `vp run build` so package-specific build steps run. Run `vp help` to print a list of commands and `vp <command> --help` for information about a specific command.
...
- Before opening a PR, run the standard Vite+ validation commands:
  `vp check`, `vp test`, and `vp run build`.
- From the workspace root, run PWA dev and preview commands with an explicit
  root: `vp dev packages/pwa` and `vp preview packages/pwa`.
